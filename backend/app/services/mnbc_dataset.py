"""Import dataset demo MNBC menjadi satu proyek CPMIS yang operasional.

Importer menerima ``files.zip`` yang diberikan pengguna, membuat proyek
MNBC-2025, task/WBS yang dapat dipakai Telegram, serta graph Digital Twin dan
contoh reasoning AI. Prosesnya idempotent: menjalankan ulang akan memperbarui
entitas dengan kunci yang sama, bukan membuat proyek duplikat.
"""
...
def load_mnbc_zip(content: bytes) -> tuple[dict, dict, list[dict]]:
    try:
    ...
    with archive:
        ...
        if missing:
            raise ValueError(
                "Dataset MNBC tidak lengkap; file wajib tidak ditemukan: "
                + ", ".join(sorted(missing))
            )
...
def _upsert_core_project(
    ...
    *,
...
) -> tuple[User, User, Project, Division, bool]:
    summary = master.get("project_summary") or {}
    project_name = summary.get("project_name") or "Pembangunan MNBC Tower"
    ...
    if not project:
        ...
        db.flush()
    project.description = (
        "Dataset demo enterprise MNBC-2025 untuk AI CPMIS, Digital Twin, "
        "pengendalian proyek, dan laporan Telegram."
    )
    ...
    field_email = "staff.mnbc@demo.local"
    field_user = db.query(User).filter(User.email == field_email).first()
    if not field_user:
        field_user = User(
            name="Staff Lapangan MNBC",
            email=field_email,
            password_hash=get_password_hash(admin_password),
            role=UserRole.STAFF,
            is_active=True,
        ...
        field_user.password_hash = get_password_hash(admin_password)
        field_user.is_active = True
    field_user.division_id = division.id
    if telegram_id:
    ...
    _upsert_membership(db, project, field_user, division, "staff")
    db.flush()
    return owner, field_user, project, division, owner_created
...
def _upsert_tasks(
    ...
    for chain in master.get("linked_chains") or []:
        ...
        if not task:
            ...
            task.specification = TaskSpecification(
                wbs_code=wbs_code,
                acceptance_criteria="Pekerjaan, mutu, volume, dan bukti lapangan sesuai dokumen proyek MNBC.",
            )
        ...
        task.assigned_to = field_user.id
        ...
        task.control.location = "MNBC Tower"
    ...
    for chain in master.get("linked_chains") or []:
        ...
        for raw in network.get("predecessors") or []:
            ...
            dependency.reason = "Diimpor dari CPM relationship dataset MNBC."
    db.flush()
    return by_activity
...
def _build_digital_twin_payload(master: dict, graph: dict, examples: list[dict]) -> DigitalTwinDatasetImport:
    ...
    for item in graph.get("nodes") or []:
        ...
        if node_type == DigitalTwinNodeType.MATERIAL and item.get("vendor"):
            ...
            relationships.append(DigitalTwinRelationshipCreate(
                ...
                reason="Vendor material pada dataset MNBC.",
            ))

    project_uid = "PRJ:MNBC-2025"
    contract_uid = "CONTRACT:MNBC-2025"
    if contract_uid not in known_uids:
        summary = master.get("project_summary") or {}
        nodes.append(DigitalTwinNodeCreate(
            uid=contract_uid,
            node_type=DigitalTwinNodeType.CONTRACT,
            name="Kontrak MNBC-2025",
            code="MNBC-2025",
    ...
    for index, edge in enumerate(graph.get("edges") or []):
        ...
        relationships.append(DigitalTwinRelationshipCreate(
            ...
            reason="Relationship dari knowledge graph MNBC.",
            metadata={
                key: value
    ...
    for index, item in enumerate(master.get("rules_engine") or [], 1):
        rule_uid = str(item.get("rule_id") or f"MNBC-RULE-{index:04d}")
        rules.append(DigitalTwinRuleCreate(
            ...
            condition_text=str(item.get("kondisi_if") or item.get("aturan") or "Kondisi aturan MNBC"),
            ...
            reference=str(item.get("sumber_standar") or "Dataset MNBC"),
            severity="medium",
    ...
    for index, item in enumerate(examples, 1):
        reasoning_examples.append(DigitalTwinReasoningExampleCreate(
            example_uid=f"MNBC-INSTRUCTION-{index:04d}",
            question=str(item.get("instruction") or "Pertanyaan MNBC"),
            context=str(item.get("input") or "Dataset proyek MNBC"),
            reasoning="Contoh jawaban terverifikasi dari dataset instruksi MNBC.",
...
def import_mnbc_demo(
    ...
    *,
    ...
    master, graph, examples = load_mnbc_zip(content)
    owner, field_user, project, division, owner_created = _upsert_core_project(
        db,
        master,
    ...
    task_map = _upsert_tasks(db, master, owner, field_user, project, division)
    db.commit()
    ...
    return {
        ...
        "field_user_email": field_user.email,
        "telegram_linked": bool(field_user.telegram_id),
